...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            a = pi.sample()
            v = self.v(state)
            _, logp_action = self.pi(state, a)
        #if a.dim() == 0:
            #a = a.item()
        return a, v, logp_action
...
class Agent:
    ...
    def v_update(self, data):
        ...
        for _ in range(100):
            ...
            val = self.ac.v.forward(torch.as_tensor(obs, dtype=torch.float32))
            # val = v.forward(obs)
            loss_function = MSELoss()
            v_loss = loss_function(val, phi)
            v_loss.backward()
