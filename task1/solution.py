...
class Model(object):
    ...
    def predict(self, test_x):
        ...
        with gpt.settings.max_preconditioner_size(10), torch.no_grad():
            with gpt.settings.max_root_decomposition_size(30), gpt.settings.fast_pred_var():
                preds = self.model(test_x)
        y = preds.loc.numpy()
        return y, preds.loc.numpy(), np.sqrt(torch.diag(preds.covariance_matrix).detach().numpy())
...
def main():
    ...
    #ax.scatter(train_x[ : , 0], train_x[ : , 1], train_y, c = train_y)
    ...
    if EXTENDED_EVALUATION:
        perform_extended_evaluation(M, output_dir='.')
