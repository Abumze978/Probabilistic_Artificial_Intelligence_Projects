...
class BO_algo(object):
    def __init__(self):
        ...
        self.solution = np.array(None) #array containing x1_opt,x2_opt,f(x_opt),c(x_opt)
        ...
        self.kernel_costraint = RBF(length_scale = 2) * ConstantKernel(constant_value = 3.5)
        self.kernel_objective = RBF(length_scale = 1.5) * ConstantKernel(constant_value = 1.5)
                
        self.constraint_model = GaussianProcessRegressor(kernel = self.kernel_costraint, alpha = 0.005) # TODO : GP model for the constraint function
        self.objective_model = GaussianProcessRegressor(kernel = self.kernel_objective, alpha = 0.01) # TODO : GP model for your objective function

        # explore exploit tradeoff coefficient
        self.xi = 0.5
    ...
    def next_recommendation(self) -> np.ndarray:
        ...
        if len(self.previous_points) <= 3:
            return np.random.uniform(0,6,(1,2))

        else:
            #We return the point that yields highest value of acquisition function
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        prev_pts = np.array(self.previous_points).reshape((-1,4))
    
        mu_sample = self.objective_model.predict(prev_pts[:,0:2])
        ...
        with np.errstate(divide='warn'):
            imp = mu - mu_sample_opt - self.xi #float
            Z = imp / sigma         
        ...
        # Probability that the constraint is satisfied (Pr)
        if sigma_c != 0:
            # pr = 1 - norm.cdf(0, loc=mu_c, scale=sigma_c)
            pr = norm.cdf(0, loc=mu_c, scale=sigma_c)
        else:
            # pr = 0.98 * (mu_c) if mu_c >= 0 else 0.02 * (- mu_c)
            pr = 0.98 if mu_c <= 0 else 0.02 
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        if self.solution.shape == ():

            if c < 0:
                self.solution = np.array([float(x[:, 0]), float(x[:, 1]), float(z), float(c)])
        else:
            
            if z < self.solution[2] and c < 0:
                self.solution = np.array([float(x[:, 0]), float(x[:, 1]), float(z), float(c)])
        

        #if self.solution.shape == () and c < 0: #first feasible value we get we necessarily put it as our best solution

           # self.solution = np.array([float(x[:, 0]), float(x[:, 1]), float(z), float(c)])

        self.previous_points.append([float(x[:, 0]), float(x[:, 1]), float(z), float(c)])

        #Adds the feasible value that has the highest value of the objective function so far
        #if not(self.solution.shape == ()) and z < self.solution[2] and c < 0: 
            
        #    self.solution = np.array([float(x[:, 0]), float(x[:, 1]), float(z), float(c)])

        #As we add points, we fit more points to our two GaussianProcessRegressors,
        #one for the objective function and one for the constraint function
        #Maybe it's not the best choice computationally speaking...

        self.objective_model.fit(np.array(self.previous_points).reshape(-1,4)[:,0:2],np.array(self.previous_points).reshape(-1,4)[:,2])
        self.constraint_model.fit(np.array(self.previous_points).reshape(-1,4)[:,0:2],np.array(self.previous_points).reshape(-1,4)[:,3])
