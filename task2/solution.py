...
class Model(object):
    ...
    def __init__(self):
        ...
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=learning_rate, weight_decay = 0.05)
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    loss, log_prior, log_variational_posterior, negative_log_likelihood = self.network.sample_elbo(batch_x, batch_y, num_batches)
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = False):
        ...
        #self.weight_prior = MultivariateDiagonalGaussian(torch.zeros(out_features, in_features), torch.ones(out_features, in_features))
        #self.bias_prior = MultivariateDiagonalGaussian(torch.zeros(out_features),torch.ones(out_features))

        PI = 0.25
        SIGMA_1 = torch.FloatTensor([math.exp(-2.3)])
        SIGMA_2 = torch.FloatTensor([math.exp(-8)])

        self.weight_prior = GaussianMixture(PI, SIGMA_1, SIGMA_2)
        self.bias_prior = GaussianMixture(PI, SIGMA_1, SIGMA_2)
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        current_features = x
        current_prior = torch.tensor(0.0)
        current_posterior = torch.tensor(0.0)

        for idx, current_layer in enumerate(self.layers):
            new_features, temp_prior, temp_posterior = current_layer.forward(current_features)
            current_prior += temp_prior
            current_posterior += temp_posterior
            if idx < len(self.layers) - 1:
                new_features = self.activation(new_features)
                current_features = new_features
            else: #last layer
                if self.training:
                    current_features = F.log_softmax(new_features,dim = 1)
                else:
                    current_features = new_features

                    

        return current_features, current_prior, current_posterior
    ...
    def sample_elbo(self, input, target, num_batches, samples=SAMPLES):
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, sigma: torch.Tensor):
        ...
        print(isinstance(mu.size(),tuple))
        print(tuple() == ())
        print(sigma.size())
...
class GaussianMixture(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        prob1 = torch.exp(self.gaussian1.log_prob(values))
        prob2 = torch.exp(self.gaussian2.log_prob(values))
